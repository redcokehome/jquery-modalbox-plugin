/*
* jQuery modalBox plugin v1.1.6 <http://code.google.com/p/jquery-modalbox-plugin/> 
	...
	*/
	...
	var defaults = {
		...
		*/
		...
		callFunctionAfterSuccess			: null, // call custom function after layer is SHOWN
		callFunctionAfterClosedLayer		: null, // call custom function after layer is CLOSED
	...
	jQuery.fn.modalBox.close = function(settings){
			...
			if( settings.fadeOutActive ){
				...
				if( settings.obsoleteIE678Browser ){
					...
					jQuery(settings.setFaderLayer).fadeOut( settings.fadeOutSpeed, function(){
						removeLayer({ container : jQuery(this) });
					});
				...
				removeLayer({ container : containerObj });
		...
		function removeLayer(options){
			...
			if( options.container ){
				...
				if( options.callFunctionAfterClosedLayer ){
					options.callFunctionAfterClosedLayer();
				}
			}
		}
